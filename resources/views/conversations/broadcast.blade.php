...
<body>
    <div class="compose-container">
        ...
        <div class="phone-input-container">
            <textarea id="phone-numbers" 
                      class="phone-input" 
                      placeholder="Enter phone numbers (comma or newline separated)&#10;&#10;Example:&#10;4065551234, 4065555678, 4065559012&#10;or&#10;406-555-1234&#10;(406) 555-5678&#10;+14065559012"
                      autocomplete="off"></textarea>
            <div class="recipient-counter">
                <span class="counter-total">📱 Recipients: <span id="total-count">0</span></span>
                <span class="counter-valid">✓ Valid: <span id="valid-count">0</span></span>
                <span class="counter-invalid">✗ Invalid: <span id="invalid-count">0</span></span>
            </div>
        </div>
        ...
        <div class="messages-area">
            <div style="text-align: center; color: #8e8e93; padding: 40px 20px;">
                <p style="font-size: 17px; margin-bottom: 10px;">📢 Broadcast SMS to Multiple Recipients</p>
                <p style="font-size: 14px; color: #666;">Enter phone numbers above and compose your message below</p>
                <p style="font-size: 12px; color: #999; margin-top: 15px;">✨ Use Quick Responses for common messages</p>
            </div>
        </div>
        ...
        <div class="compose-footer">
            ...
            <div class="quick-responses-container" id="quick-responses" style="display: none; padding: 10px; background: #f8f8f8; border-bottom: 1px solid #d1d1d6; max-height: 300px; overflow-y: auto;">
            ...
            <form id="compose-form" method="POST" action="{{ route('conversations.broadcast.send') }}" enctype="multipart/form-data" class="compose-form">
                ...
                <div class="compose-input-wrapper">
                    <!-- Quick Responses, Attach File Row -->
                ...
                <!-- Send Button -->
                <button type="submit" class="send-button" id="send-btn" disabled title="Send Broadcast">
                    ↑
                </button>
            </form>
        </div>
    ...
    <script>
        ...
        function parsePhoneNumbers(input) {
            ...
            // Clean and normalize each number
        ...
        function isValidPhoneNumber(phone) {
            ...
            // Should be 10 or 11 digits (10 for US without country code, 11 with)
            return cleaned.length === 10 || cleaned.length === 11;
        }
        
        function updatePhoneNumbers() {
            const input = document.getElementById('phone-numbers').value;
            ...
            numbers.forEach(num => {
                if (isValidPhoneNumber(num)) {
                    validNumbers.push(normalizePhoneNumber(num));
                } else {
            ...
            // Update UI
            document.getElementById('total-count').textContent = numbers.length;
            document.getElementById('valid-count').textContent = validNumbers.length;
            document.getElementById('invalid-count').textContent = invalidNumbers.length;
            
            // Update hidden field
            document.getElementById('phone-numbers-field').value = validNumbers.join(',');
        ...
        phoneNumbersInput.addEventListener('input', updatePhoneNumbers);
        ...
        messageInput.addEventListener('input', function() {
            ...
            document.getElementById('char-count').textContent = this.value.length + ' / 1600';
        ...
        function updateSendButton() {
            ...
            if (validNumbers.length > 0 && message) {
                ...
                sendBtn.title = `Send to ${validNumbers.length} recipient${validNumbers.length > 1 ? 's' : ''}`;
            } else {
                sendBtn.disabled = true;
                sendBtn.title = 'Enter recipients and message';
            }
        }
        ...
        function loadQuickResponses(filter = 'customer') {
            ...
            fetch('/api/quick-responses?filter=' + filter)
        ...
        document.getElementById('compose-form').addEventListener('submit', function(e) {
            ...
            if (invalidNumbers.length > 0) {
                if (!confirm(`You have ${invalidNumbers.length} invalid number(s). Continue sending to ${validNumbers.length} valid recipient(s)?`)) {
                    e.preventDefault();
                    return;
                }
            }
            ...
            if (validNumbers.length > 20) {
                if (!confirm(`⚠️ You are about to send this message to ${validNumbers.length} recipients.\n\nEstimated cost: $${(validNumbers.length * 0.01).toFixed(2)}\n\nContinue?`)) {
                    e.preventDefault();
                    return;
                }
            }
        ...
        // Notification System (same as compose page)
