            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.chatbot.update', $chatbotResponse) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <!-- Current Image -->
                        @if($chatbotResponse->image_url)
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    Current Image
                                </label>
                                <div class="flex items-center space-x-4">
                                    <img src="{{ $chatbotResponse->image_url }}" alt="Current image" class="h-32 w-auto rounded border">
                                    <div>
                                        <a href="{{ $chatbotResponse->image_url }}" target="_blank" class="text-blue-600 hover:text-blue-900 text-sm">
                                            🖼️ View Full Size
                                        </a>
                                        <label class="flex items-center mt-2">
                                            ...
                                            <span class="ml-2 text-sm text-red-600">Remove this image</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="mb-6">
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                                {{ $chatbotResponse->image_url ? 'Replace Image (Optional)' : 'Add Image (Optional)' }}
                            </label>
                            ...
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">
                                JPG, PNG, GIF. Max 5MB. Uploading a new image will replace the current one.
                            </p>
                        </div>
